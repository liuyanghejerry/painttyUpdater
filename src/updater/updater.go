...
func complete() (bool, error) {
	...
	err := safekill.SafeKill(int(to_be_killed))
	...
	if len(to_be_installed) <= 0 {
		log.Println("Install path is null")
		return false, nil
	}
...
func download(address string) (bool, *os.File, error) {
	log.Println("downloading new package...")
...
func init() {
	...
	// prevent killed when parent process is killed by child
	if to_be_killed != 0 {
		go func() {
			sig := make(chan os.Signal, 1)
			// be sure that we only handle SIGINT, which should generated by ourselves
			signal.Notify(sig, safekill.HandleSignal)
			<-sig // we don't really need that value
		}()
	} else {
		go func() {
			sig := make(chan os.Signal, 1)
	...
	log_file, err := os.OpenFile("./updater.log", os.O_WRONLY|os.O_APPEND|os.O_CREATE, 0755)
	if err != nil {
		fmt.Println(err)
	}
